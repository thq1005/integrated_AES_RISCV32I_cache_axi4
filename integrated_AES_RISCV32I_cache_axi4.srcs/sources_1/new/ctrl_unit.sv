...
module ctrl_unit(
	...
	assign RegWEn_o = 		(opcode_r == `OP_Stype) | // S type & B type & AES_S type
							(opcode_r == `OP_Btype) |
							(opcode_r == `OP_AES_Stype) ? (1'b0) : (1'b1);
	...
	assign AluSel_o = 		((opcode_r == `OP_Btype) | (opcode_r == `OP_JAL)   | (opcode_r == `OP_Itype_load) |
							...
							((opcode_r == `OP_Itype) & (funct3 == 3'b000)))    | (opcode_r == `OP_AES_Itype)  |
	...
	assign ImmSel_o = 	((opcode_r == `OP_Itype) | (opcode_r == `OP_JALR) | (opcode_r == `OP_Itype_load)) 	? `I_TYPE : 
						...
						((opcode_r == `OP_AES_Itype) && (funct3 == `STATUS))								? `AES_STATUS_TYPE :
						((opcode_r == `OP_AES_Itype) && (funct3 == `RESULT))								? `AES_RESULT_TYPE :
	...
	assign Valid_cpu2aes_o 	 = ((opcode_r == `OP_AES_Itype)  | (opcode_r == `OP_AES_Stype)) ? 1'b1 : 1'b0;
endmodule
