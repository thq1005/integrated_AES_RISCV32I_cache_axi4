...
module imm_gen(
	...
	assign imm_o = 		(ImmSel_i == `I_TYPE) 			? {{21{inst_i[31]}}, inst_i[30:20]} :
						...
						(ImmSel_i == `AES_BLOCK_TYPE) 	? {24{1'b0},`ADDR_BLOCK0}:
						(ImmSel_i == `AES_KEY_TYPE) 	? {24{1'b0},`ADDR_KEY0} :
						(ImmSel_i == `AES_CTRL_TYPE) 	? {24{1'b0},`ADDR_CTRL} :
						(ImmSel_i == `AES_CONFIG_TYPE) 	? {24{1'b0},`ADDR_CONFI} : 
						(ImmSel_i == `AES_STATUS_TYPE) 	? {24{1'b0},`ADDR_STATUS} : 
						(ImmSel_i == `AES_RESULT_TYPE) 	? {24{1'b0},`ADDR_RESULT} : {32{1'b0}};
